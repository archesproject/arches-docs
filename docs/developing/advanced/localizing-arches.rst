...
The above Arches management command creates a standardized PO (``.po``) file that you can translate for all the localized information in the graphs. We do not recommend using the Django supplied command ``django-admin makemessages``, because that approach requires one to hand-edit all the text values used by the Arches UI. It is easier to edit (potentially in collaboration and under version control) the PO file generated by ``python manage.py i18n makemessages``.
