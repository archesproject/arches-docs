...
* **File type checking**. Some Arches implementations may need to accept files uploaded by users. In such cases, one should configure Arches to check that uploaded files match allowed file types. This reduces risks of a user uploading a potentially damanging or dangerous file. (See :ref:`File type checking`)

* **Enable caching**. To take advantage of HTTP request rate limiting security features that reduce threats of denial-of-service type attacks, one needs to configure a cache backend for Arches. (See :ref:`Enable a Cache backend` and :ref:`Rate limiting`)
